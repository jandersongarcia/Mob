...
$path = ROOT . "\templates\\$packName";


if (APP['mode'] == 0) {
    $local = "templates\Email\\" . $app->path(2) . ".php";
    if (file_exists($local)) {
        require_once($local);
    } else {
        $error = "The subject of the email was not found.";
        require_once ('core/Php/Error.php');
    }
} else {
    $error = "Please change the 'Application Mode' to Testing by changing the string value to 0 in <span class='font-monospace'>'config/App.php'</span>.";
    require_once ('core/Php/Error.php');
}
